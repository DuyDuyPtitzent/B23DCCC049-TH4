...
const FormDatLich: React.FC<FormDatLichProps> = ({ visible, onCancel, edit = false }) => {
  ...
  const { record, formSubmiting, postModel, putModel, kiemTraLichTrung } = useModel('datlich.lichhen', (model: any) => ({
    record: model.record,
    formSubmiting: model.formSubmiting,
    postModel: model.postModel,
    putModel: model.putModel,
    kiemTraLichTrung: model.kiemTraLichTrung,
  }));
  ...
  const onFinish = async (values: any) => {
    ...
    if (edit) {
      putModel(record?._id ?? '', lichHenData)
        ...
        .catch((err: unknown) => { // Thêm kiểu 'unknown' cho tham số 'err'
      ...
      postModel(lichHenData)
        ...
        .catch((err: unknown) => console.log(err)); // Thêm kiểu 'unknown' cho tham số 'err'
    }
  };
  return (
    <Modal
